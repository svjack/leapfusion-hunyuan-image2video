...
def sample_image2video(model, x, sigmas, extras, extra_args=None, callback=None, disable=None, eta=1., s_noise=1., noise_sampler=None):
    ...
    prompt_embeds, prompt_mask, prompt_embeds_2, freqs_cos, freqs_sin, guidance_expand, return_dict, accelerator, timesteps, num_inference_steps, seed = extras#t.repeat(latents.shape[0]).to(device=device, dtype=dit_dtype)
...
def main():
    ...
    if args.latent_path is not None and len(args.latent_path) > 0:
        ...
        latents = sample_image2video(transformer, latents, scheduler.sigmas, (prompt_embeds, prompt_mask, prompt_embeds_2, freqs_cos, freqs_sin, guidance_expand, True, accelerator, timesteps, num_inference_steps, seed))
